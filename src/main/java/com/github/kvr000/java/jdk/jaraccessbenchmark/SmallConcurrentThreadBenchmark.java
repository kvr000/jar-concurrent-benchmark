...
public class SmallConcurrentThreadBenchmark
{
	...
	{
		try (InputStream input = Files.newInputStream(Paths.get("src/main/resources/com/github/kvr000/java/jdk/jaraccessbenchmark/small.txt"))) {
	...
	{
		try (InputStream input = jarFile.getInputStream(jarFile.getEntry("com/github/kvr000/java/jdk/jaraccessbenchmark/small.txt"))) {
			ByteArrayOutputStream bytes = new ByteArrayOutputStream();
			input.transferTo(bytes);
			blackhole.consume(bytes);
		}
	...
	{
		try (InputStream input = SmallSingleThreadBenchmark.class.getClassLoader().getResourceAsStream("com/github/kvr000/java/jdk/jaraccessbenchmark/small.txt")) {
			ByteArrayOutputStream bytes = new ByteArrayOutputStream();
			input.transferTo(bytes);
			blackhole.consume(bytes);
		}
	...
	{
		try (InputStream input = SmallSingleThreadBenchmark.class.getResourceAsStream("small.txt")) {
			ByteArrayOutputStream bytes = new ByteArrayOutputStream();
			input.transferTo(bytes);
			blackhole.consume(bytes);
		}
