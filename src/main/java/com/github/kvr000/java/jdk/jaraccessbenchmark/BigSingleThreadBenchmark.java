...
public class BigSingleThreadBenchmark
{
	...
	{
		try (InputStream input = BigSingleThreadBenchmark.class.getClassLoader().getResourceAsStream("com/github/kvr000/java/jdk/jaraccessbenchmark/angular.js")) {
			ByteArrayOutputStream bytes = new ByteArrayOutputStream();
			input.transferTo(bytes);
			blackhole.consume(bytes);
		}
	...
	{
		try (InputStream input = BigSingleThreadBenchmark.class.getResourceAsStream("angular.js")) {
			ByteArrayOutputStream bytes = new ByteArrayOutputStream();
			input.transferTo(bytes);
			blackhole.consume(bytes);
		}
